...
* **Recommended fix:** Check the suspend notification immediately after actuations as well (or replace long `vTaskDelay`s with `xTaskNotifyWait(..., timeout)` so the suspend request itself wakes the task and ends the actuation early, turning the relay off before suspending). Then a short timeout is honest. Show a “Switching…” screen during the wait.
