...
### BUG-020 — Single temperature sensor is a SPOF; DHT22’s temperature channel is unused for cross-checking; heater has no minimum-cycle or maximum-runtime guard ✅ FIXED
...
* **Fix applied:** (1) `task_sensor` now calls `dht.readTemperature()` alongside humidity; the result is stored as `temp_dht` / `dht_temp_valid` in `SensorData_t`. (2) `task_temperature_control` reads both fields and runs a **cross-sensor plausibility check**: if |T_ds18b20 − T_dht| > `SENSOR_PLAUSIBILITY_DELTA_C` (5 °C) for more than `SENSOR_PLAUSIBILITY_HOLD_MS` (5 min), a throttled `pushError("SENSOR_WARN", …)` fires. (3) A **heater max-on watchdog** tracks `heaterOnSince`; if the heater has been continuously ON for more than `HEATER_MAX_ON_MS` (30 min) without a temperature rise of at least `HEATER_RISE_THRESHOLD_C` (0.5 °C), a throttled `pushError("HEATER_WARN", …)` fires. All thresholds are named constants in `config.h`.

### BUG-021 — Heap fragmentation risk from `String` churn in the cloud task; token leaked into logs ✅ FIXED
* **Severity:** Medium
...
|----|---------|----------|-------------|
| BUG-022 ✅ | Retry path treats any HTTP code > 0 (incl. 404/500) as success, while the first-attempt path checks `code != 200` — inconsistent server-error handling | [task_cloud.cpp:78-80](egg_incubator_v2/task_cloud.cpp#L78-L80) | Retry path now requires `code >= 200 && code < 400`; 404/500 etc. schedule a retry or drop, matching the first-attempt criterion. |
| BUG-023 ✅ | `t == 85.0f` exact float compare rejects a legitimate 85.0 °C reading — relevant in climate profile whose valid range extends to 100 °C; also masks a real over-limit reading as “sensor invalid” instead of over-temp | [task_sensors.cpp:32](egg_incubator_v2/task_sensors.cpp#L32) | 85.0 is treated as POR only when implausible: no prior valid reading, or last valid reading outside 80–90 °C. A genuine 85 °C in the climate profile (preceded by readings near it) is now accepted — and the >80 °C latch fires first anyway. |
| BUG-024 ✅ | Errors are dropped silently when offline: `errorQueue` (20 deep) is drained only while Wi-Fi is connected; `pushError` drops on full with no counter, and the OLED never displays non-fault errors | [globals.cpp:48-56](egg_incubator_v2/globals.cpp#L48-L56), [task_cloud.cpp:96-99](egg_incubator_v2/task_cloud.cpp#L96-L99) | Added `std::atomic<uint32_t> gErrorsDropped`; `pushError` increments it on a full queue. After each error-queue drain, `task_cloud` exchanges the counter to 0 and pushes one `ERR_DROPPED` summary (“N errors dropped (queue full)”). |
| BUG-025 ✅ | Dead UI code: `UI_SET_ENV_MENU` and `UI_SETTINGS_MENU` handlers are unreachable (no state transition enters them); `UI_EVT_LONGOK` is defined but never generated by `task_buttons` | [task_ui.cpp:351-448, 1474-1530](egg_incubator_v2/task_ui.cpp#L351-L1530) | Removed both unreachable handlers, the `UI_SET_ENV_MENU`/`UI_SETTINGS_MENU` enum values, the `SettingsMenuItem` enum, the `envMenuIdx`/`settingsMenuIdx` statics, and `UI_EVT_LONGOK`. Sketch shrank ~900 B. |
| BUG-026 ✅ | `setFanSpeed()` lazy LEDC init guarded by a non-atomic `static bool initialized`, callable concurrently from UI (via `allRelaysOff`), control and fan tasks → double `ledc_*_config` race | [task_incubator.cpp:162-190](egg_incubator_v2/task_incubator.cpp#L162-L190) | Init extracted into `initFanPwm()`, called once from `setup()` immediately after the relay pins are driven OFF — before any task exists. `setFanSpeed()` now only computes/applies duty. |
| BUG-027 ✅ | Fault screen shows `-999.0 C` when the sensor is invalid during an over-temp latch | [task_ui.cpp:167-173](egg_incubator_v2/task_ui.cpp#L167-L173), [oled_ui.cpp:291-294](egg_incubator_v2/oled_ui.cpp#L291-L294) | `oled_show_fault_screen()` prints `--.-` when `currentTemp < -100`. |
